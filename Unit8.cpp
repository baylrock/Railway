...
Form8->Image1->Visible = false;
        ...
        {
                ComboBox1->Items->Add(DM->sql_filter_pas->FieldByName("Фамилия")->AsString );
...
//робота з МС Ворд
/*
pas="Пасажир : "+DM->mod_bilet->FieldByName("пасажир")->AsString;
napr="Напрямок руху :"+DM->mod_bilet->FieldByName("направление")->AsString;
tr="Потяг : "+DM->mod_bilet->FieldByName("номер_поезда")->AsString;
tv="Тип вагону : "+DM->mod_bilet->FieldByName("тип_вагона")->AsString;
nv="Номер вагону : "+DM->mod_bilet->FieldByName("номер_вагона")->AsString;
pl="Місце : "+DM->mod_bilet->FieldByName("место")->AsString;
po="Відправлення : "+DM->mod_bilet->FieldByName("пункт_отпр")->AsString;
pp="Прибуття : "+DM->mod_bilet->FieldByName("пункт_приб")->AsString;
da="Дата відправлення : "+DM->mod_bilet->FieldByName("дата_отпр")->AsString;
va="Час відправлення : "+DM->mod_bilet->FieldByName("время_отпр")->AsString;
dp="Дата прибуття : "+DM->mod_bilet->FieldByName("дата_приб")->AsString;
vp="Час прибуття : "+DM->mod_bilet->FieldByName("время_приб")->AsString;
ss="Сервіс : "+DM->mod_bilet->FieldByName("сервис")->AsString;
km="Кілометраж : "+DM->mod_bilet->FieldByName("километраж")->AsString;
st="Вартість : "+DM->mod_bilet->FieldByName("стоимость")->AsString;
*/

pas=DM->mod_bilet->FieldByName("пасажир")->AsString;
napr=DM->mod_bilet->FieldByName("направление")->AsString;
tr=DM->mod_bilet->FieldByName("номер_поезда")->AsString;
tv=DM->mod_bilet->FieldByName("тип_вагона")->AsString;
nv=DM->mod_bilet->FieldByName("номер_вагона")->AsString;
pl=DM->mod_bilet->FieldByName("место")->AsString;
po=DM->mod_bilet->FieldByName("пункт_отпр")->AsString;
pp=DM->mod_bilet->FieldByName("пункт_приб")->AsString;
da=DM->mod_bilet->FieldByName("дата_отпр")->AsString;
va=DM->mod_bilet->FieldByName("время_отпр")->AsString;
dp=DM->mod_bilet->FieldByName("дата_приб")->AsString;
vp=DM->mod_bilet->FieldByName("время_приб")->AsString;
ss=DM->mod_bilet->FieldByName("сервис")->AsString;
km=DM->mod_bilet->FieldByName("километраж")->AsString;
st=DM->mod_bilet->FieldByName("стоимость")->AsString;
...
void __fastcall TForm8::ComboBox1Select(TObject *Sender)
{
         if (ComboBox1->Text.Length()!=0)
        {
                ...
                DM->sql_filter_pas->Locate("фамилия",ComboBox1->Text, Options);
        }
        
        if (ComboBox1->Text.Length()>0)
        {
                DM->mod_bilet->Filtered = false;
                DM->mod_bilet->Filter = "код_пас = " + DM->sql_filter_pas->FieldByName("код_пас")->AsString;
